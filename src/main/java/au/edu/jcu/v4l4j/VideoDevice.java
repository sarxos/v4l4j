 ...
 * </ul>
 * Each of these categories is detailed in the following sections. To use
 * features provided in any of these categories, a <code>VideoDevice</code>
 * object must first be instantiated. This is done simply by calling the
 * constructor and giving it the full path to the associated device file: <br>
 * <code>
 ...
 * method:</b> <br>
 * <code>vd.release();</code> <br>
 * Once the <code>VideoDevice</code> is released, neither itself nor any object
 * references obtained through it must be used. <h2>Information gathering</h2> A
 * <code>VideoDevice</code> object offers a single method (
 * {@link #getDeviceInfo()}) to return information about itself, encapsulated in
 * a {@link DeviceInfo} object. DeviceInfo objects contain get-type methods
 * only, and return details such as:
 * <ul>
 ...
 * {@link V4L4JException} if a process is already using the device. <h2>
 * Capturing frames</h2> In order to capture frames from a video device, you
 * must first obtain an object implementing the {@link FrameGrabber} interface.
 * The {@link VideoDevice} objects provides several methods for that, depending
 * on the image format required:
 * <ul>
 * 
 * <li>The <code>getJPEGFrameGrabber()</code> method returns a
 * {@link JPEGFrameGrabber} object capable of capturing frames and converting
 * them to JPEG before handing them out. This frame grabber cannot be used with
 * all video sources. It requires images from the video source to be in some
 * pre-defined formats in order to be converted to JPEG. If the video source is
 * not capable of handing out images in one of the supported formats, then no
 ...
 * all video sources. They require images from the video source to be in some
 * pre-defined formats in order to be converted to RGB24 or BGR24. If the video
 * source is not capable of handing out images in one of the supported formats,
 * then no <code>RGBFrameGrabber</code> / <code>BGRFrameGrabber</code> can be
 * created.</li>
 * 
 ...
 * before handing them out. Here again, these frame grabbers cannot be used with
 * all video sources. They require images from the video source to be in some
 * pre-defined formats in order to be converted to YUV420 or YVU420. If the
 * video source is not capable of handing out images in one of the supported
 * formats, then no <code>YUVFrameGrabber</code> / <code>YVUFrameGrabber</code>
 * can be created.</li>
 * 
 ...
 * calling the {@link #releaseFrameGrabber()}. See the {@link FrameGrabber}
 * interface for more information on how to capture frames.
 * 
 * <h2>Tuners</h2> A list of available tuners can be obtained by calling the
 ...
 * 
 * <h2>Video controls</h2> A list of available video controls can be obtained by
 * calling the {@link #getControlList()} method. Each control contains detailed
 * information about its type and acceptable values. Controls have a pair of get
 * & set methods to adjust their values. Once again, when no longer used,
 * resources taken up by video controls must be released by calling
...
public class VideoDevice {
	...
	/**
	 * Whether or not frames captured from this video device can be JPEG, RGB24,
	 * BGR24, YUV420 or YVU420-encoded
	 */
	 ...
	 *            {@link #releaseFrameGrabber()} or/and
	 *            {@link #releaseControlList()} have been called
	 * @throws StateException if this video device has already been released, or
	 *             is being released.
	 * @throws ReleaseException if there either the
	 *             {@link #releaseFrameGrabber()} or the
	 *             {@link #releaseControlList()} have not been called, and we
	 *             asked to not wait (argument <code>wait</code> is false)
	 */
	 ...
	 * @throws V4L4JException if there was an error gathering information about
	 *             the video device. This happens for example, when another
	 ...
	 * device. The {@link ControlList} must be released when no longer used by
	 * calling {@link VideoDevice#releaseControlList()}.
	 * 
	 ...
	 * {@link #getRGBFrameGrabber(int, int, int, int, ImageFormat)} will
	 * succeed. If this method returns false,no <code>RGBFrameGrabber</code>s
	 * can be instantiated. One alternative is to use a raw frame grabber,
	 * returned by {@link #getRawFrameGrabber(int, int, int, int)} or
	 * {@link #getRawFrameGrabber(int, int, int, int, ImageFormat)}.<br>
	 ...
	 * {@link #getBGRFrameGrabber(int, int, int, int, ImageFormat)} will
	 * succeed. If this method returns false,no <code>BGRFrameGrabber</code>s
	 * can be instantiated. One alternative is to use a raw frame grabber,
	 * returned by {@link #getRawFrameGrabber(int, int, int, int)} or
	 * {@link #getRawFrameGrabber(int, int, int, int, ImageFormat)}.<br>
	 ...
	 * {@link #getYUVFrameGrabber(int, int, int, int, ImageFormat)} will
	 * succeed. If this method returns false, no <code>YUVFrameGrabber</code>s
	 * can be instantiated. One alternative is to use a raw frame grabber,
	 * returned by {@link #getRawFrameGrabber(int, int, int, int)} or
	 * {@link #getRawFrameGrabber(int, int, int, int, ImageFormat)}.<br>
	 * <b>If a YUVFrameGrabber cannot be created for your video device, please
	 * let the author know about it so YUV-encoding can be added. See the README
	 ...
	 * {@link #getYVUFrameGrabber(int, int, int, int, ImageFormat)} will
	 * succeed. If this method returns false, no <code>YVUFrameGrabber</code>s
	 * can be instantiated. One alternative is to use a raw frame grabber,
	 * returned by {@link #getRawFrameGrabber(int, int, int, int)} or
	 * {@link #getRawFrameGrabber(int, int, int, int, ImageFormat)}.<br>
	 * <b>If a YVUFrameGrabber cannot be created for your video device, please
	 * let the author know about it so YVU-encoding can be added. See the README
	 * file on how to submit reports.</b>
	 * 
	 * @return whether or not frames captured by this video device can be
	 *         YVU420-encoded.
	...
	/**
	 * This method returns a {@link JPEGFrameGrabber} associated with this video
	 * device. Captured frames will be JPEG-encoded before being handed out. To
	 * obtain a JPEGFrameGrabber, this video device must support an appropriate
	 * image format that v4l4j can convert to JPEG. If it does not, this method
	 * will throw an {@link ImageFormatException}. To check if JPEG-encoding is
	 * possible, call {@link #supportJPEGConversion()}. The list of image
	 * formats supported by this video device that can be JPEG-encoded is
	 * obtained by calling {@link ImageFormatList#getJPEGEncodableFormats()} (
	 ...
	 * used by calling {@link #releaseFrameGrabber()}.<br>
	 * <b>If JPEGFrameGrabbers cannot be created for your video device, please
	 ...
	 *            before being JPEG-encoded. This image format must be one that
	 *            v4l4j can convert to JPG, ie it must be in the list returned
	 *            by this video device's
	 *            {@link ImageFormatList#getJPEGEncodableFormats()}. You can get
	 *            this video device's {@link ImageFormatList} by calling
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, v4l4j will pick the first image format
	 *            it knows how to JPEG-encode.
	 * @return a {@link JPEGFrameGrabber} associated with this video device, if
	 *         supported.
	 ...
	 *             JPEG-encoded. If no image format was specified (
	 *             <code>null</code>), then this video device does not have an
	 *             image format that can be JPEG encoded. <b>If you encounter
	 *             such device, please let the author know so support for it can
	 *             be added. See the README file on how to submit reports.</b>
	 ...
	 *             be released before a JPEGFrameGrabber can be allocated, or if
	...
	/**
	 * This method returns a {@link JPEGFrameGrabber} associated with this video
	 * device. Captured frames will be JPEG-encoded before being handed out. The
	 * video device must support an appropriate image format that v4l4j can
	 * convert to JPEG. If it does not, this method will throw an
	 * {@link ImageFormatException}. To check if JPEG-encoding is possible, call
	 * {@link #supportJPEGConversion()}. Among all the image formats the video
	 * device supports, v4l4j will choose the first one that can be JPEG
	 ...
	 *             formats which can be JPEG-encoded.<b>If you encounter such
	 *             device, please let the author know so support for it can be
	 *             added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 * @throws ImageDimensionException if the given image dimensions are not
	 *             supported
	...
	/**
	 * This method returns a {@link RGBFrameGrabber} associated with this video
	 * device. Captured frames will be converted to RGB24 before being handed
	 ...
	 * {@link ImageFormatException}. To check if RGB24 conversion is possible,
	 * call {@link #supportRGBConversion()}. The returned
	 ...
	 *            {@link V4L4JConstants})
	 * @param imf the {@link ImageFormat} the frames should be captured in
	 *            before being converted to RGB24. This image format must be one
	 *            that v4l4j can convert to RGB24, ie it must be in the list
	 *            returned by this video device's
	 *            {@link ImageFormatList#getRGBEncodableFormats()}. You can get
	 *            this video device's {@link ImageFormatList} by calling
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, v4l4j will pick the first image format
	 *            it can RGB24-encode.
	 ...
	 *             supported
	 * @throws ImageFormatException if the chosen image format can not be
	 *             RGB24-encoded. If no image format was specified (
	 *             <code>null</code>), then this video device does not have an
	 *             image format that can be RGB24-encoded. <b>If you encounter
	 *             such device, please let the author know so support for it can
	 *             be added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists and must
	 *             be released before another FrameGrabber can be allocated, or
	...
	/**
	 * This method returns a {@link RGBFrameGrabber} associated with this video
	 * device. Captured frames will be RGB24-encoded before being handed out.
	 * The video device must support an appropriate image format that v4l4j can
	 * convert to RGB24. If it does not, this method will throw an
	 ...
	 *             format that can be RGB24-encoded. <b>If you encounter such
	 *             device, please let the author know so support for it can be
	 *             added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists and must
	 *             be released before a RGBFrameGrabber can be allocated, or if
	 *             the <code>VideoDevice</code> has been released.
	 */
	public RGBFrameGrabber getRGBFrameGrabber(int w, int h, int input, int std)
	...
	/**
	 * This method returns a {@link BGRFrameGrabber} associated with this video
	 * device. Captured frames will be converted to BGR24 before being handed
	 * out. The video device must support an appropriate image format that v4l4j
	 * can convert to BGR24. If it does not, this method will throw an
	 * {@link ImageFormatException}. To check if BGR24 conversion is possible,
	 * call {@link #supportBGRConversion()}. The returned
	 ...
	 *            {@link V4L4JConstants})
	 * @param imf the {@link ImageFormat} the frames should be captured in
	 *            before being converted to BGR24. This image format must be one
	 *            that v4l4j can convert to BGR24, ie it must be in the list
	 *            returned by this video device's
	 *            {@link ImageFormatList#getBGREncodableFormats()}. You can get
	 *            this video device's {@link ImageFormatList} by calling
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, v4l4j will pick the first image format
	 *            it can BGR24-encode.
	 ...
	 *             supported
	 * @throws ImageFormatException if the chosen image format can not be
	 *             BGR24-encoded. If no image format was specified (
	 *             <code>null</code>), then this video device does not have an
	 *             image format that can be BGR24-encoded. <b>If you encounter
	 *             such device, please let the author know so support for it can
	 *             be added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	...
	/**
	 * This method returns a {@link BGRFrameGrabber} associated with this video
	 * device. Captured frames will be BGR24-encoded before being handed out.
	 * The video device must support an appropriate image format that v4l4j can
	 * convert to BGR24. If it does not, this method will throw an
	 ...
	 *             supported
	 * @throws ImageFormatException if this video device does not have an image
	 *             format that can be BGR24-encoded. <b>If you encounter such
	 *             device, please let the author know so support for it can be
	 *             added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists and must
	 *             be released before a BGRFrameGrabber can be allocated, or if
	 *             the <code>VideoDevice</code> has been released.
	 */
	public BGRFrameGrabber getBGRFrameGrabber(int w, int h, int input, int std)
	throws V4L4JException {
	...
	/**
	 * This method returns a {@link YUVFrameGrabber} associated with this video
	 * device. Captured frames will be converted to YUV420 before being handed
	 * out. The video device must support an appropriate image format that v4l4j
	 * can convert to YUV420. If it does not, this method will throw an
	 * {@link ImageFormatException}. To check if YUV420 conversion is possible,
	 * call {@link #supportYUVConversion()}. The returned
	 * {@link YUVFrameGrabber} must be released when no longer used by calling
	 * {@link #releaseFrameGrabber()}.<br>
	 * <b>If YUVFrameGrabbers cannot be created for your video device, please
	 ...
	 *            {@link V4L4JConstants})
	 * @param imf the {@link ImageFormat} the frames should be captured in
	 *            before being converted to YUV420. This image format must be
	 *            one that v4l4j can convert to YUV420, ie it must be in the
	 *            list returned by this video device's
	 *            {@link ImageFormatList#getYUVEncodableFormats()}. You can get
	 *            this video device's {@link ImageFormatList} by calling
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, v4l4j will pick the first image format
	 *            it can YUV420-encode.
	 * @return a {@link YUVFrameGrabber} associated with this video device, if
	 *         supported.
	 * @throws VideoStandardException if the chosen video standard is not
	 *             supported
	 * @throws ImageFormatException if the chosen image format can not be
	 *             YUV420-encoded. If no image format was specified (
	 *             <code>null</code>), then this video device does not have an
	 *             image format that can be YUV420-encoded. <b>If you encounter
	 *             such device, please let the author know so support for it can
	 *             be added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	...
	/**
	 * This method returns a {@link YUVFrameGrabber} associated with this video
	 * device. Captured frames will be YUV420-encoded before being handed out.
	 * The video device must support an appropriate image format that v4l4j can
	 * convert to YUV420. If it does not, this method will throw an
	 ...
	 *             supported
	 * @throws ImageFormatException if this video device does not have an image
	 *             format that can be YUV420-encoded. <b>If you encounter such
	 *             device, please let the author know so support for it can be
	 *             added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists and must
	 *             be released before a YUVFrameGrabber can be allocated, or if
	 *             the <code>VideoDevice</code> has been released.
	 */
	public YUVFrameGrabber getYUVFrameGrabber(int w, int h, int input, int std)
	throws V4L4JException {
	...
	/**
	 * This method returns a {@link YVUFrameGrabber} associated with this video
	 * device. Captured frames will be converted to YVU420 before being handed
	 * out. The video device must support an appropriate image format that v4l4j
	 * can convert to YVU420. If it does not, this method will throw an
	 * {@link ImageFormatException}. To check if YVU420 conversion is possible,
	 * call {@link #supportYVUConversion()}. The returned
	 * {@link YVUFrameGrabber} must be released when no longer used by calling
	 * {@link #releaseFrameGrabber()}.<br>
	 * <b>If YVUFrameGrabbers cannot be created for your video device, please
	 ...
	 *            {@link V4L4JConstants})
	 * @param imf the {@link ImageFormat} the frames should be captured in
	 *            before being converted to YVU420. This image format must be
	 *            one that v4l4j can convert to YVU420, ie it must be in the
	 *            list returned by this video device's
	 *            {@link ImageFormatList#getYVUEncodableFormats()}. You can get
	 *            this video device's {@link ImageFormatList} by calling
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, v4l4j will pick the first image format
	 *            it can YVU420-encode.
	 * @return a {@link YVUFrameGrabber} associated with this video device, if
	 *         supported.
	 * @throws VideoStandardException if the chosen video standard is not
	 *             supported
	 * @throws ImageFormatException if the chosen image format can not be
	 *             YVU420-encoded. If no image format was specified (
	 *             <code>null</code>), then this video device does not have an
	 *             image format that can be YVU420-encoded. <b>If you encounter
	 *             such device, please let the author know so support for it can
	 *             be added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 *             initialised
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link AbstractGrabber} already exists and
	 *             must be released before another FrameGrabber can be
	 *             allocated, or if the <code>VideoDevice</code> has been
	 *             released.
	 */
	...
	/**
	 * This method returns a {@link YVUFrameGrabber} associated with this video
	 * device. Captured frames will be YVU420-encoded before being handed out.
	 * The video device must support an appropriate image format that v4l4j can
	 * convert to YVU420. If it does not, this method will throw an
	 ...
	 *             supported
	 * @throws ImageFormatException if this video device does not have an image
	 *             format that can be YVU420-encoded. <b>If you encounter such
	 *             device, please let the author know so support for it can be
	 *             added. See the README file on how to submit reports.</b>
	 * @throws CaptureChannelException if the given channel number value is not
	 *             valid
	 ...
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists and must
	 *             be released before a YVUFrameGrabber can be allocated, or if
	 *             the <code>VideoDevice</code> has been released.
	 */
	public YVUFrameGrabber getYVUFrameGrabber(int w, int h, int input, int std)
	throws V4L4JException {
	...
	/**
	 * This method returns a {@link RawFrameGrabber} associated with this video
	 * device. Captured frames will be handed out in the same format as received
	 * from the driver and can be chosen amongst the ones supported by this
	 * video device. To enumerate the supported {@link ImageFormat}s, check the
	 * {@link ImageFormatList} returned by {@link DeviceInfo#getFormatList()}.
	 ...
	 *            <code>getDeviceInfo().getFormatList()</code>. If this argument
	 *            is <code>null</code>, an {@link ImageFormatException} is
	 *            thrown.
	 ...
	 *             initialised
	 * @throws V4L4JException if there is an error applying capture parameters
	 * @throws StateException if a {@link FrameGrabber} already exists or if the
	 ...
	 * This method returns a {@link RawFrameGrabber} associated with this video
	 * device. Captured frames will be handed out in the same format as received
	 * from the driver. The chosen format is the one returned by
	 * <code>getDeviceInfo().getFormatList().getNativeFormats().get(0)</code>.
	 ...
	 *             the {@link DeviceInfo} object could not be obtained. Check if
	 *             the device is not already used by another application.
	 * @throws V4L4JException if there is an error applying capture parameters
	 ...
	 * <code>getRawFrameGrabber()</code>. This method must be called when the
	 * <code>FrameGrabber</code> object is no longer used, so low-level
	 * resources can be freed. This method does nothing if no
	 ...
	 * for more information. This method (as well as {@link #getDeviceInfo()}
	 * does not have an equivalent release method. In other word, the returned
	 * {@link TunerList} object does not need to be released before releasing
	 * the {@link VideoDevice}.
	 * 
	...
	/**
	 * This method sets the {@link ThreadFactory} to be used when new threads
	 * are created by frame grabbers created for this video device. By default,
	 * a video device instance will use the thread factory returned by
	 * <code>Executors#defaultThreadFactory()</code>. If this is not suitable,
	 * you can specify your own thread factory using this method.
	 * 
	 * @param factory the {@link ThreadFactory} to use when creating new
	 *            threads. If <code>null</code>, the default factory returned by
